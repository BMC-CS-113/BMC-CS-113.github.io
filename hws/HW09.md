...
Your task is to create a solution that meets these requirements. You'll be using object-oriented programming principles to create classes for `Pets`, and some specific type of pets (detailed in `HW09Driver.java`).
...
The clinic has given you a driver program called `HW09Driver.java` that should work after you implement the required classes.

The text file `driverResults.txt` shows what the results of running `java HW09Driver` should be.

You can find the three files by appending them to 
this url: `{{ site.url }}{{ site.baseurl }}/hws/hw09/`.

> **WARNING:** You are **NOT** allowed to edit `HW09Driver.java` or `Database.java` 
...
the assignment called `HW09` on Gradescope. Do not upload
`HW09Driver.java` or `Database.java`.
